...
const Navbar = () => {
  ...
  const toggleResume = () => {
    const resumeUrl = "https://drive.google.com/file/d/1tw1sfT2nZ4vM1ohk2Jn4pmXY1HEwUmSJ/view?usp=sharing";
    ...
    <ul className={`list-none ${isSecondary ? "flex sm:hidden" : "hidden sm:flex"} flex-row gap-6`}>
      {navLinks.map((link) => (
        <li
          key={link.id}
          className={`${
            active === link.title ? "text-secondary" : isSecondary ? "text-white" : "text-secondary"
